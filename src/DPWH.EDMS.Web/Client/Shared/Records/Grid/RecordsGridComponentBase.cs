...
namespace DPWH.EDMS.Web.Client.Shared.Records.Grid;

public class RecordsGridComponentBase : GridBase<RecordModel>
{
    ...
    protected override async Task LoadData(bool bPageChanged = false)
    {
        ...
        try
        {
            ...
            {
                result = await RecordManagementService.QueryByEmployeeId(EmployeeId, DataSourceReq);
            }
            else
            {
                result = await RecordManagementService.Query(DataSourceReq);
            }

            // Convert the retrieved data to a list of BookingModel objects
            GridData = GenericHelper.GetListByDataSource<RecordModel>(result.Data);
