...
public sealed class CreateAssetValidator : AbstractValidator<CreateAssetCommand>
{
    ...
    {
        RuleFor(command => command.CreateAssetRequest)
            .NotNull()
            ...
            .ChildRules(v =>
            {
                ...
                v.RuleFor(param => param.YearConstruction)
                    .NotEmpty()
                    .Must(year => year is >= 1900 && year <= DateTime.Now.Year)
                    .WithMessage("Year of Construction must not be empty or null and not less than 1900 and not greater than the current year.");
